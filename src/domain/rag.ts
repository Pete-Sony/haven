...
import { APPROVED_CLAIMS, type ApprovedClaimId } from "@/domain/resources";
...
export const supportMemoryActionIdSchema = z.enum(SUPPORT_MEMORY_ACTION_IDS);
...
export interface PersonalSupportContext {
  ...
  readonly actionId: SupportMemoryActionId;
...
export type RagRetrievalResult =
  | { readonly status: "ready"; readonly context: RagContext }
  | { readonly status: "emergency_bypass" }
  | {
      readonly status: "grounding_unavailable";
      readonly reason: "educational_evidence_not_found";
    };

const EVIDENCE_SOURCE_BY_ROUTE = {
  individual: {
    urgent_support: "haven.craving-support.v1",
    coping: "haven.craving-support.v1",
  },
  caregiver: {
    urgent_support: "haven.caregiver-talk.v1",
    coping: "haven.caregiver-talk.v1",
  },
} as const satisfies Readonly<
  Record<Role, Record<Exclude<RiskTier, "emergency">, ApprovedClaimId>>
>;
...
function retrieveEducation(
  ...
  const sourceId = EVIDENCE_SOURCE_BY_ROUTE[input.role][decision.tier];
  const evidence = APPROVED_CLAIMS[sourceId];
  if (
    !evidence.enabled ||
    !(evidence.roles as readonly Role[]).includes(input.role) ||
    !(evidence.tiers as readonly Exclude<RiskTier, "emergency">[]).includes(
      decision.tier,
    ) ||
    Date.parse(evidence.recheckAt) < now.getTime()
  ) {
    return null;
  }
  return evidence;
...
export function retrieveRagContext(
  ...
  if (decision.tier === "emergency") {
    return { status: "emergency_bypass" };
  }
  ...
  return { status: "ready", context: { educational, personal } };
}
